...
function displayAgent(jsonAgent) {
    ...
    p.appendChild(addCellButton("Make Transaction", function () {
    	/*$('#id01').show();
        makeTransaction(agent.name);*/
        showPopup(agent.name);
    ...
    row.insertCell(idx++).innerHTML = agent.balance;
   /* const nameCell = row.insertCell(idx++);
    nameCell.title = agent.name;*/
    //nameCell.innerHTML = agent.balance;
}
...
function displayBlock(jsonBlock) {
    ...
    displayMsg("New Transaction Created:<br>" + getBlockString(block), "green");
}
...
function getBlockString(block) {
    return "index=" + block.index + " creator=" + block.creator + " timestamp="
        ...
        +"sender=" + block.sender + "receiver=" + block.reciever + "amount= "+ block.amount;
}
...
function cleanTable(name) {
    ...
    row.insertCell(idx++).innerHTML = "Wallet";
